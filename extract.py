...
# Attempt to reconstruct media paths, set to False if you get errors while extracting
TRY_PATH_RECONSTRUCT = True
# If one of the headers is corrupted, these can help
FORCE_NEW_HEADER = True
FORCE_OLD_HEADER = True
...
def getDataIndex(start):
    ...
    if start > datalen:
        raise Exception("Data starts outside of loaded files: " + str(start) + "/" + str(datalen))
...
# --- New helper function to sanitize XML ---
def sanitize_xml(xml_str):
    # Replace numeric character references with empty string if they are invalid.
    def replace_invalid(match):
        num = int(match.group(1))
        # Valid XML 1.0 character ranges:
        if num in (0x9, 0xA, 0xD) or (0x20 <= num <= 0xD7FF) or (0xE000 <= num <= 0xFFFD) or (0x10000 <= num <= 0x10FFFF):
            return match.group(0)
        else:
            return ""
    return re.sub(r'&#(\d+);', replace_invalid, xml_str)
...
def readHeader(isNew):
    ...
    assert len(res) == size
    res2 = decrypt(res)
    # Save the header file for inspection
    ...
    # Decode, sanitize, and then parse the XML header
    xml_decoded = res2.decode("utf-8", errors="ignore")
    clean_xml = sanitize_xml(xml_decoded)
...
if __name__ == "__main__":
    ...
    if subFileSize is not None and int(subFileSize.text) > 1:
        ...
        print("[+] Opened " + str(count + 1) + " backup files, total size: " + str(datalen))
        # Adjust usedList offsets for multipart backups
        shift = dataSizes[-2]
    ...
    for item in sorted(usedList, key=lambda x: x[0]):
        if item[0] > pos:
            ...
            print("Overlap!: pos: " + str(pos) + " start: " + str(item[0]) + " len: " + str(item[1]))
            continue
        pos = item[0] + item[1]
    if pos < datalen:
        ln = datalen - pos
